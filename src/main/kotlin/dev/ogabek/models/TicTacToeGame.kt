...
class TicTacToeGame {
    ...
    fun finishTurn(player: Char, x: Int, y: Int) {
        ...
        if (state.value.playerTurn != player) {
            return
        }

        val currentPlayer = state.value.playerTurn
        ...
        state.update {
            ...
            it.copy(
                playerTurn = if (currentPlayer == 'X') 'O' else 'X',
                field = newField,
                isBoardFull = isBoardFull,
                winningPlayer = getWinnerPlayer()?.also {
                    startNewRoundDelayed()
                }
            )
        }
    ...
    private fun getWinnerPlayer(): Char? {
